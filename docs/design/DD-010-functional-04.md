...
On phones the idle study action is integrated beside the Today heading, with statistics alongside it. An active timer uses one compact row under the heading. The date controls stay on one row and now/next uses reduced spacing and typography, preserving its event information. There is no floating overlay covering the timetable. Browser checks require the first timetable entry to be visible at 320/360/390 px and retain usable active-timer controls.
