...
try{
 ...
 await page.locator('[data-individual=manage]').first().click();await page.locator('[data-edit-selection]').first().click();await page.locator('#individual-choice [name=subject]').fill('Мой математический анализ');const excluded=await page.locator('#individual-choice [name=from]').inputValue();await page.locator('#individual-choice [name=excludeDates]').fill(excluded);await page.locator('#individual-choice .primary').click();await page.reload();await page.locator('#dialog [data-action=close]').click();await page.locator('[data-nav=more]').first().click();
 await page.locator('[data-individual=manage]').first().click();await page.locator('[data-toggle-selection]').click();saved=await page.evaluate(()=>JSON.parse(localStorage.getItem('setka.unstable.v1')));assert.equal(saved.personalSelections[0].subject,'Мой математический анализ');assert.equal(saved.personalSelections[0].disabled,true);assert.deepEqual(saved.personalSelections[0].excludeDates,[excluded]);assert.equal(saved.tasks.length,0);
