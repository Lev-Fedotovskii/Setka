...
export function occurrences(schedule, groupId, date) {
  ...
  return schedule.series.filter(s => s.cohorts.some(c=>c.groupId===groupId) && date >= s.recurrence.validFrom && date <= s.recurrence.validTo && !s.recurrence.excludeDates?.includes(date) && (s.recurrence.includeDates?.includes(date) || (s.recurrence.weekdays.includes(weekday(date)) && (s.recurrence.parity==='all' || s.recurrence.parity===parity)))).map(s=>({...s, id:`${s.id}@${date}`,seriesId:s.id,date,start:minute(s.time.startsAt),end:minute(s.time.endsAt),type:'lesson'})).sort((a,b)=>a.start-b.start || a.id.localeCompare(b.id));
}
