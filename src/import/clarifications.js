...
export function timingClarification(series,term){
  if(term?.id!=='mipt-2026-autumn'||series.source?.workbook!=='БВО 1 КУРС ОСЕНЬ 2026-2027 г..xlsx'||series.source.sheet!=='Лист1'||series.source.ranges?.length!==1||!['CK55:CK57','CK56:CK58'].includes(series.source.ranges[0])||series.source.rawText!=='Введение в программирование, уч.асс. Гавва А.С.-706 КПМ'||series.cohorts?.length!==1||series.cohorts[0].groupId!=='Б06-603'||series.recurrence?.weekdays?.length!==1||series.recurrence.weekdays[0]!==4||series.time?.startsAt!=='15:30'||series.time.endsAt!=='18:30')return null;
